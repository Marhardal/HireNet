            ...
            <div class="flex-1">
                <div class="text-center mx-auto">
                    <h3 class="my-3 text-2xl font-semibold w-full">
                        {{ $resumes->user->first_name . ' ' . $resumes->user->surname }}
                    </h3>
                    <div class="text-lg font-semibold gap-4 flex justify-center">
                        {{-- <h5>{{ $resumes->district . ', ' . $resumes->country }}</h5> --}}
                        <h5>{{ $resumes->user->phone }}</h5>
                        <h5>{{ $resumes->user->email }}</h5>
                    </div>
                </div>
                ...
                <hr class="">
                <p class="my-2 text-base">{{ $resumes->summary }}</p>
                <div class="flex w-full text-base my-2">
                    <h3 class="text-lg font-semibold flex-auto">Work History</h3>
                </div>
                <hr>
                <div class="grid grid-cols-4 gap-5">
                    @foreach ($resumes->jobs as $item1)
                ...
                @if ($resumes->skills != '')
                    <ul class="mx-auto grid max-w-6xl  grid-cols-5 gap-1 p-1">
                        @foreach ($resumes->skills as $item)
                ...
                <hr>
                @if ($resumes->certificates != '')
                    <div class="gap-5 my-2 w-full grid grid-cols-2">
                        @foreach ($resumes->certificates as $item)
                            <div class="col-span-1">
                                <div class="text-base">
                ...
                <div class="grid grid-cols-3">
                    @if ($resumes->referrals->count() > 0)
                        @foreach ($resumes->referrals as $item)
